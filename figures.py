...
class Sphere(object):
    ...
    def ray_intersect(self, orig, dir):
        L = ml.subtract_twoVectorsOfSameSize(self.center, orig)
        # L = np.subtract(self.center, orig)
        tca = ml.dot_twoVectorsOfSameSize(L, dir)
        # tca = np.dot(L, dir)
        ...
        # P = O + t0 * D
        P1 = ml.vectorMultiplyByNumber(dir, t0)
        P = ml.sum_twoVectorsOneByThree(orig, P1)
        # P = np.add(orig, t0 * np.array(dir))
        normal = ml.subtract_twoVectorsOfSameSize(P, self.center)
        # normal = np.subtract(P, self.center)
        normal = ml.vectorDivideByNumber(normal, ml.norm(normal))
...
class Plane(object):
    def __init__(self, position, normal,  material):
        ...
        self.normal = ml.vectorDivideByNumber(normal, ml.norm(normal))
        # self.normal = normal / np.linalg.norm(normal)
        self.material = material

    def ray_intersect(self, orig, dir):
        # Distancia = (( planePos - origRayo) o normal) / (direccionRayo o normal)
        denom = ml.dot_twoVectorsOfSameSize(dir, self.normal)
        # denom = np.dot( dir, self.normal)

        if abs(denom) > 0.0001:
            num = ml.dot_twoVectorsOfSameSize(ml.subtract_twoVectorsOfSameSize(self.position, orig), self.normal)
            # num = np.dot( np.subtract(self.position, orig), self.normal)
            t = num / denom

            if t > 0:
                # P = O + t*D
                P1 = ml.vectorMultiplyByNumber(dir, t)
                P = ml.sum_twoVectorsOneByThree(orig, P1)
...
class Disk(object):
    ...
    def ray_intersect(self, orig, dir):
        ...
        contact = ml.subtract_twoVectorsOfSameSize(intersect.point, self.plane.position)
...
class AABB(object):
    ...
    def __init__(self, position, size, material):
        ...
        # Sides
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (halfSizes[0],0,0)), (1,0,0), material ))
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (-halfSizes[0],0,0)), (-1,0,0), material ))
        # self.planes.append( Plane( np.add(position, (halfSizes[0],0,0)), (1,0,0), material ))
        # self.planes.append( Plane( np.add(position, (-halfSizes[0],0,0)), (-1,0,0), material ))

        # Up and Down
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (0,halfSizes[1],0)), (0,1,0), material ))
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (0,-halfSizes[1],0)), (0,-1,0), material ))
        # self.planes.append( Plane( np.add(position, (0,halfSizes[1],0)), (0,1,0), material ))
        # self.planes.append( Plane( np.add(position, (0,-halfSizes[1],0)), (0,-1,0), material ))

        # Front and back
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (0,0,halfSizes[2])), (0,0,1), material ))
        self.planes.append( Plane( ml.sum_twoVectorsOneByThree(position, (0,0,-halfSizes[2])), (0,0,-1), material ))
